...
class GridWorld:
    def __init__(self, rows, cols, walls=[], pits=[], goals=[], live_reward=0.0, esc=[], escf=[], serp=[],serpf=[]):
        ...
        self.action_probabilities2 = [0.8, 0.2]
        
        self.action_probs_cum = np.cumsum(self.action_probabilities)
        self.action_probs_cum2 = np.cumsum(self.action_probabilities2)
        
        self.r_init_render()

    # inicializar valores de estado v(s) en 0 para todos los estados
    ...
    # inicializar valores de par estado-acción q(s,a) en 0 para todos los estados
...
#         return state_values, state_q_values
    ...
    def step2(self, state, action, random=False):
        
        if(state in self.goals): # estado terminal diamante
            return "Terminal Diamante", +1.0, None, True
        elif(state in self.pits): # estado terminal bomba
            return "Terminal Bomba", -1.0, None, True
        else:
            assert action in self.get_allowed_actions(state)
            
            if random: # estocastico
                rand = np.random.uniform()
                if rand<=self.action_probs_cum2[0]:
                    real_action = self.real_actions2[action][0]
                else:
                    real_action = self.real_actions2[action][1]
            else: # determinisitco
                real_action = action
    ...
    def solve_value_iteration2(self, gamma=1.0, horizon=10, init_state=(2,0)):
        escape = False
        for i in range(horizon):
            self.r_draw_background()
            self.r_draw_values(show_action=True)
...
#                 self.state_q_values = state_q_values_1
    ...
    def follow_policy(self, init_state=(-1,-1), flag_q=False, gamma=1.0):
        ...
        while True:
            ...
            if done:
                break
            print('estaba en el estado',state)
            ...
            if state in self.goals or state in self.pits:
                action = self.policy[state]
                ...
                print('saque en el dado: ',dado)
                if self.policy[state]==a[state[0]][state[1]]:
                    
                    for j in range(dado-1):
                        ...
                        if a[newst[0]][newst[1]]=='E':
                            newst = (newst[0], newst[1]+1)
                    if newst in self.goals or newst in self.pits:
                        if a[newst[0]][newst[1]]=='N':
                            newst = (newst[0]-1, newst[1])
                        if a[newst[0]][newst[1]]=='S':
                            newst = (newst[0]+1, newst[1])
                    ...
                    if newst in self.goals or newst in self.pits:
                        ...
                        state=newst
                        action = r[state[0]][state[1]]
                        state, reward, _, done = self.step(state, action, random=False)                    
                        utility += (gamma**i)*-0.1
                        i += 1
            print('llegue a ',state)
