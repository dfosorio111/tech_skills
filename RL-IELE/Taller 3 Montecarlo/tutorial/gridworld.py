...
class GridWorld:
    ...
    def init_qvalues2(self):
        state_q_values = {}
        for state in self.states:
            state_q_values[state] = dict.fromkeys(['a','r'], 0.0)
        return state_q_values
...
#                 self.state_q_values = state_q_values_1
    ...
    def generainador(self, init_state=(-1,-1), gamma=1.0):
        ...
        while True:
            ...
            if state in self.goals or state in self.pits:
                action = self.policy[state]
                state, reward, _, done = self.step(state, action, random=False)
                if state=="Terminal Bomba":
                    rewards_epi.append(-1)
                elif state=="Terminal Diamante":
                    rewards_epi.append(10)
                utility += (gamma**i)*reward
                i += 1
            else:
                ...
                if random.choice(['a','r'])=='a':
                    actions_epi.append('a')
                    for j in range(dado-1):
                        if a[newst[0]][newst[1]]=='N':
                            newst = (newst[0]-1, newst[1])
                        if a[newst[0]][newst[1]]=='S':
                    ...
                    if newst in self.goals or newst in self.pits:
                        ...
                        if a[newst[0]][newst[1]]=='E':
                            newst = (newst[0], newst[1]+1)
                        utility += (gamma**i)*self.live_reward
                        rewards_epi.append(self.live_reward)
                        i += 1
                    else:
                        state=newst
                        action = a[state[0]][state[1]]
        ...
        return rewards_epi,actions_epi,states_epi,f
    
    
    
    def jugainador(self, init_state=(-1,-1), flag_q=False, gamma=1.0):
